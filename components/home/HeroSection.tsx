...
import {
  ...
  Clock,
  ChevronDown,
  ArrowRight // ArrowRight ikonu əlavə edildi
} from 'lucide-react';
...
export function HeroSection() {
  ...
  return (
    <section className="hero-bg min-h-screen flex items-center relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-20 w-full">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-white space-y-8 animate-fade-in">
            <div className="space-y-4">
              ...
              <p className="text-xl text-white/80 max-w-lg leading-relaxed"> {/* Subtitle rəngi və aralığı dəyişdirildi */}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                Rent Cars & Services
                <ArrowRight className="h-5 w-5 ml-2" /> {/* Arrow icon əlavə edildi */}
              </Button>
              <Button
                size="lg"
                variant="outline" // variant="ghost" yerinə variant="outline" istifadə edildi
                className="border-white text-white hover:bg-white/10 px-8 py-6 text-lg hover-scale" // Şəffaf fon, ağ kənar
              >
                Find Auto Services
                <ArrowRight className="h-5 w-5 ml-2" /> {/* Arrow icon əlavə edildi */}
              </Button>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 pt-8">
              {stats.map((stat, index) => (
                <div key={index} className="bg-white/10 border border-white/20 rounded-2xl p-4 text-center hover-scale"> {/* Fon və kənar dəyişdirildi */}
                  <stat.icon className="h-8 w-8 text-destructive mx-auto mb-2" /> {/* İkon rəngi qırmızı edildi */}
                  <div className="text-3xl font-bold">{stat.value}</div> {/* Rəqəm ölçüsü artırıldı */}
                  <div className="text-sm text-white/80">{stat.label}</div> {/* Mətn rəngi ağ edildi */}
                </div>
              ))}
            </div>
          ...
          <div className="relative animate-slide-up">
            <div className="bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl hover-scale"> {/* Fon və kənar dəyişdirildi */}
              <img
              ...
              />
              <div className="absolute top-12 right-12 bg-white/10 border border-white/20 rounded-xl p-3"> {/* Fon və kənar dəyişdirildi */}
                <div className="text-white text-center">
                  <div className="text-sm text-white/70">Starting from</div>
                  <div className="text-2xl font-bold text-destructive">AED 89</div> {/* Rəng qırmızı edildi */}
